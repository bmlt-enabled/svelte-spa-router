...
For advanced cases where you want to restore scroll position yourself (for example, only on certain routes), the router exports a `restoreScroll` function. It takes an optional history-state object containing the previously-saved scroll coordinates; if no state is passed, it scrolls to the top of the page.
